...
const createOrder = async (req, res) => {
  try {
    ...
    const invalidItems = selectedItems.filter(item => !item.book || !item.book.seller);
    if (invalidItems.length > 0) {
      console.error('Invalid book or seller data:', invalidItems.map(item => ({
        bookId: item.book?._id,
        hasSeller: !!item.book?.seller
      })));
      return res.status(400).json({ 
        message: "Some books in your cart have invalid seller information. Please remove them and try again or buy from another seller." 
      });
    }
